...
import { BUILTIN_MATERIALS, BUILTIN_MATERIAL_BY_ID, slotMaterialKey } from '@/engine/materials/builtinMaterials'
...
function Swatch({ kind, index, hex }: { kind: PaletteSlotKind; index: number; hex: string }) {
  ...
  const materialName = materialId ? (BUILTIN_MATERIAL_BY_ID[materialId]?.name ?? null) : null
 ...
 *
 * The special-material rows (emissive/metal/glass, then carpaint) are laid out as
 * spacer + 4 + spacer + 4 + spacer + 4 + spacer — 4 spacer slots + 12 swatches = 16 slots,
 * exactly matching the 16-wide base row above, so all rows share the same total width and
 * center perfectly on top of each other. The carpaint row recenters with 6 + 4 + 6.
 */
/**
 ...
 * Sphere thumbnails are the libraries' own renders, vendored with the maps (see
 * `public/materials/NOTICE.md`). Horizontally scrolls; the leading "none" circle clears.
 */
  ...
  return (
    <div className="flex max-w-100 items-center gap-2 overflow-x-auto py-1">
      <button
        ...
        title="No material (plain color)"
        onClick={() => setSlotMaterial(slotKey, null)}
        className={
          `${SPHERE} flex items-center justify-center rounded-full ring-2 transition-transform hover:scale-110 ` +
      ...
      {BUILTIN_MATERIALS.map((m) => {
        ...
        return (
          <button
            ...
            aria-label={m.name}
            title={m.name}
            onClick={() => setSlotMaterial(slotKey, active ? null : m.id)}
            className={
    ...
    >
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex items-center gap-1">
          ...
          <div className={SWATCH} />
          <div className={SWATCH} />
          {palette.carpaint.map((hex, index) => (
            <Swatch key={index} kind="carpaint" index={index} hex={hex} />
          ))}
          <div className={SWATCH} />
          <div className={SWATCH} />
          <div className={SWATCH} />
